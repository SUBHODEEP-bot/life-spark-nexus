      ...
      <Dialog open={showAssistantDialog} onOpenChange={setShowAssistantDialog}>
        <DialogContent className="sm:max-w-md max-h-[80vh] overflow-hidden flex flex-col dropdown-content">
          ...
          <div className="flex-1 overflow-y-auto py-4 space-y-4 max-h-[50vh]">
            {chatHistory.map((item, index) => <div key={index} className={`flex ${item.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[80%] rounded-lg p-3 text-sm ${item.role === 'user' ? 'bg-lifemate-purple text-white' : 'bg-secondary/70'}`}>
                  {item.message}
                </div>
              </div>)}
            
            {assistantTyping && <div className="flex justify-start">
          ...
          <div className="mt-4 flex gap-2">
            <input type="text" value={userQuery} onChange={e => setUserQuery(e.target.value)} placeholder="Type your question here..." className="flex-1 p-2 rounded-md border border-border bg-background focus:outline-none focus:ring-1 focus:ring-lifemate-purple" onKeyDown={e => {
